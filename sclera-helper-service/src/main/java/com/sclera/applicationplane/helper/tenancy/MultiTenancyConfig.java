 ...
 * migrations across every ACTIVE schema â€” a failed tenant is logged, not fatal
 * for the fleet.
 */
@Configuration
        ...
        return args -> {
            ...
            for (String schema : registry.listActiveSchemas()) {
                try {
                    ...
                    log.error("Tenant schema migration failed for {} â€” continuing with the rest", schema, e);
                }
            }
        };
    }
